...
def extension(extensions: list):
    extensions_dict = {}
    for word in extensions:
        split_word = word.split('.')
        try:
            extensions_dict[split_word[1]] += 1
        except KeyError:
            extensions_dict[split_word[1]] = 1
    extensions_tuple = list(extensions_dict.items())
    max_encounter_amount = max(item[1] for item in extensions_tuple)
    result = []
    for word in range(0, len(extensions_tuple)):
        if extensions_tuple[word][1] == max_encounter_amount:
            result.append('.' + extensions_tuple[word][0])
